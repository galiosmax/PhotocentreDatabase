...
class BranchOfficeDao(private val dataSource: DataSource) {
    ...
    fun updateBranchOffice(branchOffice: BranchOffice) {
        val statement = dataSource.connection.prepareStatement("UPDATE branch_offices SET branch_office_address = ?, branch_office_amount_of_workers = ? WHERE branch_office_id = ?")
        statement.setString(1, branchOffice.address)
        statement.setInt(2, branchOffice.amountOfWorkers)
    ...
    fun deleteBranchOffice(id: Long) {
        val statement = dataSource.connection.prepareStatement("DELETE FROM branch_offices WHERE branch_office_id = ?")
        statement.setLong(1, id)
        statement.executeUpdate()
    }
}